...
async function listNotes() {
  ...
  // 自动清除：超过 clearAfter 天、且未被保留的文件删除（clearAfter=0 表示永久保留）
  if (cfg.clearAfter > 0) {
    const now = Date.now()
    for (const sub of SUBDIRS) {
      if (sub === '归档') continue // 归档目录不自动清除
      let entries = []
      try {
      ...
      for (const ent of entries) {
        ...
        if (!m) continue
        const created = new Date(Number(m[1]), Number(m[2]) - 1, Number(m[3])).getTime()
        const ageDays = (now - created) / 86400000
        if (ageDays > cfg.clearAfter) {
          try { await rm(join(root, sub, ent.name), { force: true }) } catch (e) { /* ignore */ }
        }
      }
    }
  }
