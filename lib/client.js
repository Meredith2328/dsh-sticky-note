...
/* 标准右下角缩放手柄：始终可见，鼠标与触摸共用 Pointer Events */
.sn-rz-corner {
  position: absolute;
  right: 2px;
  bottom: 2px;
...
.sn-rz-corner:hover::after { opacity: 1; }
@keyframes sn-pop {
  from { opacity: 0; transform: scale(var(--sn-zoom-pop, 0.94)) translateY(6px); }
  to { opacity: 1; transform: scale(var(--sn-zoom, 1)) translateY(0); }
}
.sn-head {
  position: relative;
  display: flex;
...
.sn-head-title { flex: none; white-space: nowrap; }
.sn-drag-grip {
  color: var(--dsw-alias-label-primary, rgba(0, 0, 0, 0.72));
  width: 20px;
  height: 22px;
  flex: none;
  margin: 0 4px;
  ...
  border-radius: 6px;
  background-color: var(--dsw-alias-interactive-bg-hover-solid, rgba(0, 0, 0, 0.09));
  background-image: radial-gradient(circle, currentColor 1.8px, transparent 1.9px);
  background-size: 8px 7px;
  background-position: 2px 1px;
  cursor: grab;
}
.sn-drag-grip:active { cursor: grabbing; }
.sn-head-right { display: flex; align-items: center; flex: none; }
.sn-head-actions-pop {
  position: absolute;
  z-index: 40;
  top: 5px;
  ...
  border-radius: 9px;
  background: var(--dsw-specific-input-major, rgba(255, 255, 255, 0.96));
  box-shadow: 0 4px 14px rgba(0, 0, 0, 0.14);
}
body[data-ds-dark-theme] .sn-head-actions-pop { background: rgba(38, 38, 40, 0.98); }
.sn-actions-toggle { letter-spacing: 1px; font-weight: 700; }
.sn-zoom-controls {
  display: flex;
  align-items: center;
  gap: 2px;
...
.sn-pin-toggle { flex: none; }
.sn-pin-toggle svg { width: 12px; height: 12px; }
.sn-panel.sn-pinned { border-color: var(--dsw-alias-button-info-fill, #3b82f6); }
.sn-x {
  border: 1px solid var(--dsw-alias-border-l2-darkmode-thin, rgba(0, 0, 0, 0.12));
...
.sn-bar-3col { position: relative; padding-right: 36px; }
.sn-bar-center {
  position: absolute;
  left: 50%;
...
`
    ...
    function renderMarkdown(src) {
      ...
      while (i < lines.length) {
        ...
        // 空行
        if (!trim) { closeList(); out.push('<div class="sn-md-blank" aria-hidden="true">&nbsp;</div>'); i++; continue }
    ...
    const PANEL_LAYOUT_KEY = 'sn-panel-layout-v1'
    const PANEL_PINNED_KEY = 'sn-panel-pinned-v1'
    const PANEL_ZOOM_KEY = 'sn-panel-zoom-v1'
    const PANEL_MARGIN = 4
    const PANEL_MIN_WIDTH = 240
    const PANEL_MIN_HEIGHT = 180
    const DEFAULT_PANEL_LAYOUT = { x: null, y: null, width: 280, height: 330 }
    const PANEL_ZOOM_MIN = 80
    const PANEL_ZOOM_MAX = 150
    const PANEL_ZOOM_STEP = 10

    function parsePanelLayout(raw, legacySizeRaw) {
      try {
        if (raw) {
          const value = JSON.parse(raw)
          if (value && Number.isFinite(value.x) && Number.isFinite(value.y)
            && Number.isFinite(value.width) && Number.isFinite(value.height)
            && value.width >= PANEL_MIN_WIDTH && value.width <= 10000
            && value.height >= PANEL_MIN_HEIGHT && value.height <= 10000) {
            return { x: value.x, y: value.y, width: value.width, height: value.height }
          }
        }
      } catch (e) { /* ignore damaged storage */ }
      try {
        if (legacySizeRaw) {
          const value = JSON.parse(legacySizeRaw)
          if (value && Number.isFinite(value.width) && Number.isFinite(value.height)
            && value.width >= PANEL_MIN_WIDTH && value.width <= 10000
            && value.height >= PANEL_MIN_HEIGHT && value.height <= 10000) {
            return { x: null, y: null, width: value.width, height: value.height }
          }
        }
      } catch (e) { /* ignore damaged legacy storage */ }
      return { ...DEFAULT_PANEL_LAYOUT }
    }

    function parsePanelZoom(raw) {
      const value = Number(raw)
      if (!Number.isFinite(value) || value < PANEL_ZOOM_MIN || value > PANEL_ZOOM_MAX) return 100
      return Math.round(value / PANEL_ZOOM_STEP) * PANEL_ZOOM_STEP
    }

    function panelZoomCommand(event) {
    ...
    function nextPanelZoom(current, command) {
      if (command === 'reset') return 100
      if (command === 'in') return Math.min(PANEL_ZOOM_MAX, current + PANEL_ZOOM_STEP)
      if (command === 'out') return Math.max(PANEL_ZOOM_MIN, current - PANEL_ZOOM_STEP)
      return current
    }

    function clampPanelLayout(layout, viewport, zoom = 100) {
      const scale = parsePanelZoom(zoom) / 100
      const maxWidth = Math.max(0, (viewport.width - PANEL_MARGIN * 2) / scale)
      const maxHeight = Math.max(0, (viewport.height - PANEL_MARGIN * 2) / scale)
      ...
      const maxX = Math.max(PANEL_MARGIN, viewport.width - width * scale - PANEL_MARGIN)
      const maxY = Math.max(PANEL_MARGIN, viewport.height - height * scale - PANEL_MARGIN)
      const x = Math.max(PANEL_MARGIN, Math.min(Number.isFinite(layout.x) ? layout.x : PANEL_MARGIN, maxX))
      const y = Math.max(PANEL_MARGIN, Math.min(Number.isFinite(layout.y) ? layout.y : PANEL_MARGIN, maxY))
      return { x, y, width, height }
    }

    function snapPanelLayout(layout, viewport, zoom = 100) {
      const scale = parsePanelZoom(zoom) / 100
      const next = clampPanelLayout(layout, viewport, zoom)
      const left = PANEL_MARGIN
      const right = Math.max(PANEL_MARGIN, viewport.width - next.width * scale - PANEL_MARGIN)
      const center = next.x + next.width * scale / 2
      return { ...next, x: center <= viewport.width / 2 ? left : right }
    }
    ...
    function StickyNoteApp(props) {
      ...
      // 默认关闭，保持现有打开行为；用户在设置中可选择继续上次草稿。
      const [resumeDraft, setResumeDraft] = React.useState(false)
      // 保留视图（选择哪些便签不被自动清除）
      const [retainMode, setRetainMode] = React.useState(false)
      // 帮助弹窗
      const [helpOpen, setHelpOpen] = React.useState(false)
      // 标题栏操作默认收起；展开层覆盖标题栏右侧，不再挤压标题文字。
      const [headActionsOpen, setHeadActionsOpen] = React.useState(false)
      ...
      const initialLayout = (() => {
        try {
          return parsePanelLayout(localStorage.getItem(PANEL_LAYOUT_KEY), localStorage.getItem('sn-panel-size'))
        } catch (e) { return { ...DEFAULT_PANEL_LAYOUT } }
      })()
      const initialPinned = (() => {
        try { return localStorage.getItem(PANEL_PINNED_KEY) === 'true' } catch (e) { return false }
      })()
      const initialZoom = (() => {
        try { return parsePanelZoom(localStorage.getItem(PANEL_ZOOM_KEY)) } catch (e) { return 100 }
      })()
      ...
      const preferredLayoutRef = React.useRef(initialLayout)
      ...
      function persistLayout() {
        try { localStorage.setItem(PANEL_LAYOUT_KEY, JSON.stringify(preferredLayoutRef.current)) } catch (e) { /* ignore */ }
      }
      ...
      function togglePinned() {
        ...
        try { localStorage.setItem(PANEL_PINNED_KEY, String(nextPinned)) } catch (e) { /* ignore */ }
        if (nextPinned) {
          const snapped = snapPanelLayout(layoutRef.current, viewport(), zoomRef.current)
          preferredLayoutRef.current = snapped
          showLayout(snapped)
          persistLayout()
        }
        setStatus(nextPinned ? '已固定到边缘' : '已取消固定')
      }

      function setPanelZoom(nextZoom) {
        const next = parsePanelZoom(nextZoom)
        zoomRef.current = next
        setZoom(next)
        try { localStorage.setItem(PANEL_ZOOM_KEY, String(next)) } catch (e) { /* ignore */ }
        const nextLayout = pinnedRef.current
          ? snapPanelLayout(layoutRef.current, viewport(), next)
          : clampPanelLayout(layoutRef.current, viewport(), next)
        preferredLayoutRef.current = nextLayout
        showLayout(nextLayout)
        persistLayout()
        setStatus('缩放 ' + next + '%')
      }

      function changePanelZoom(command) {
        setPanelZoom(nextPanelZoom(zoomRef.current, command))
      }

      function measurePanel() {
        if (!fabRef.current) return
        const r = fabRef.current.getBoundingClientRect()
        const preferred = preferredLayoutRef.current
        const scale = zoomRef.current / 100
        const desired = Number.isFinite(preferred.x) && Number.isFinite(preferred.y)
          ? preferred
          : { ...preferred, x: r.right - preferred.width * scale - 4, y: r.top - preferred.height * scale - 8 }
        const next = pinnedRef.current
          ? snapPanelLayout(desired, viewport(), zoomRef.current)
          : clampPanelLayout(desired, viewport(), zoomRef.current)
        preferredLayoutRef.current = next
        showLayout(next)
      }

      function onPanelPointerDown(e, mode) {
      ...
      React.useEffect(() => {
        function onMove(e) {
          ...
          const scale = zoomRef.current / 100
          const desired = d.mode === 'move'
            ? { ...d.start, x: d.start.x + dx, y: d.start.y + dy }
            : { ...d.start, width: d.start.width + dx / scale, height: d.start.height + dy / scale }
          const next = clampPanelLayout(desired, viewport(), zoomRef.current)
          preferredLayoutRef.current = next
          showLayout(next)
        }
        function onUp(e) {
          ...
          if (!d || e.pointerId !== d.pointerId) return
          try {
            if (d.target.releasePointerCapture && d.target.hasPointerCapture && d.target.hasPointerCapture(d.pointerId)) {
              d.target.releasePointerCapture(d.pointerId)
            }
          } catch (error) { /* capture may already be released */ }
          dragRef.current = null
          if (pinnedRef.current) {
            const snapped = snapPanelLayout(preferredLayoutRef.current, viewport(), zoomRef.current)
            preferredLayoutRef.current = snapped
            showLayout(snapped)
          }
          persistLayout()
        ...
        function onResize() {
          const next = pinnedRef.current
            ? snapPanelLayout(preferredLayoutRef.current, viewport(), zoomRef.current)
            : clampPanelLayout(preferredLayoutRef.current, viewport(), zoomRef.current)
          preferredLayoutRef.current = next
          showLayout(next)
        }
      ...
      React.useEffect(() => {
        rpc('config', {}).then((v) => {
          ...
          if (typeof v.resumeDraft === 'boolean') setResumeDraft(v.resumeDraft)
        }).catch(() => {})
        return () => {
      ...
      // 仅在用户主动开启时恢复宿主记录的当前草稿；“+”仍显式开启新便签。
      React.useEffect(() => {
        if (!open || !resumeDraft) return
        let cancelled = false
        rpc('draft', {}).then((draft) => {
          if (cancelled || !draft || typeof draft.content !== 'string') return
          setText(draft.content)
          textRef.current = draft.content
          if (draft.kind && ['点子', '感想', 'TODO'].includes(draft.kind)) {
            setKind(draft.kind)
      ...
      React.useEffect(() => {
        if (!open) setHeadActionsOpen(false)
      }, [open])

      React.useEffect(() => {
        if (!headActionsOpen) return
        function onDocMouseDown(e) {
          const target = e.target
          if (target && target.closest && (target.closest('.sn-head-actions-pop') || target.closest('.sn-actions-toggle'))) return
          setHeadActionsOpen(false)
        }
        document.addEventListener('mousedown', onDocMouseDown)
        return () => document.removeEventListener('mousedown', onDocMouseDown)
      }, [headActionsOpen])

      // 默认点击外部关闭；固定后忽略外部点击，显式关闭仍然有效。
      React.useEffect(() => {
      ...
      function onPanelZoomKeyDown(e) {
        const command = panelZoomCommand(e)
        if (!command) return
        e.preventDefault()
        e.stopPropagation()
        changePanelZoom(command)
      }
      ...
      function onKeyDown(e) {
        ...
        if (e.key === 'Escape') {
          e.preventDefault()
          if (headActionsOpen) { setHeadActionsOpen(false); return }
      ...
      const panel = React.createElement('div', {
        ref: panelRef,
        className: 'sn-panel' + (pinned ? ' sn-pinned' : ''),
        onKeyDownCapture: onPanelZoomKeyDown,
        style: {
          ...
          '--sn-zoom': zoom / 100,
          '--sn-zoom-pop': zoom / 100 * 0.94,
        },
      },
        // 头部：右侧操作默认收起，展开后以浮层覆盖标题栏右侧，避免挤压标题。
        React.createElement('div', { className: 'sn-head', title: '拖动便签', onPointerDown: (e) => onPanelPointerDown(e, 'move') },
          React.createElement('span', null,
            viewNote ? null : React.createElement('button', { className: 'sn-plus', title: '新建便签', 'aria-label': '新建便签', onClick: newNote }, '+'),
            React.createElement('span', { className: 'sn-drag-grip', title: '拖动便签', 'aria-hidden': true }),
            React.createElement(NoteIcon, null),
            React.createElement('span', { className: 'sn-head-title' }, viewNote ? '查看' : (view === 'list' ? '历史便签' : '便签')),
            status ? React.createElement('span', { className: 'sn-headstatus' }, status) : null,
          ),
          React.createElement('span', { className: 'sn-head-right' },
            headActionsOpen
              ? React.createElement('span', { className: 'sn-head-actions-pop' },
                  React.createElement('span', { className: 'sn-zoom-controls' },
                    React.createElement('button', {
                      className: 'sn-mini',
                      title: '缩小便签（Cmd/Ctrl+-）',
                      'aria-label': '缩小便签',
                      disabled: zoom <= PANEL_ZOOM_MIN,
                      onClick: () => changePanelZoom('out'),
                    }, '−'),
                    React.createElement('button', {
                      className: 'sn-mini sn-zoom-value',
                      title: '恢复 100%（Cmd/Ctrl+0）',
                      'aria-label': '重置便签缩放，当前 ' + zoom + '%',
                      onClick: () => changePanelZoom('reset'),
                    }, zoom + '%'),
                    React.createElement('button', {
                      className: 'sn-mini',
                      title: '放大便签（Cmd/Ctrl++）',
                      'aria-label': '放大便签',
                      disabled: zoom >= PANEL_ZOOM_MAX,
                      onClick: () => changePanelZoom('in'),
                    }, '+'),
                  ),
                  React.createElement('button', {
                    className: 'sn-mini sn-pin-toggle' + (pinned ? ' sn-on' : ''),
                    title: pinned ? '取消固定（允许点击外部关闭）' : '固定到屏幕边缘',
                    'aria-label': pinned ? '取消固定便签' : '固定便签',
                    'aria-pressed': pinned,
                  ...
                  React.createElement('button', {
                    className: 'sn-mini sn-actions-toggle sn-on',
                    title: '收起操作',
                    'aria-label': '收起标题栏操作',
                    'aria-expanded': true,
                    onClick: () => setHeadActionsOpen(false),
                  }, '⋯'),
                )
              : React.createElement('button', {
                  className: 'sn-mini sn-actions-toggle',
                  title: '更多操作',
                  'aria-label': '展开标题栏操作',
                  'aria-expanded': false,
                  onClick: () => setHeadActionsOpen(true),
                }, '⋯'),
          ),
        ),
        ...
        React.createElement('button', {
          type: 'button',
          className: 'sn-rz-corner',
          title: '拖动调整便签大小',
          'aria-label': '调整便签大小',
          onPointerDown: (e) => onPanelPointerDown(e, 'resize'),
        }),
    ...
    function SettingsCard(props) {
      ...
      return React.createElement('li', { className: 'sn-set-card' },
        ...
        open ? React.createElement('div', { className: 'sn-set-body' },
          ...
          OptionGroup('重新打开便签时', [
            { value: false, label: '保持现在的行为' },
            { value: true, label: '继续上次草稿' },
          ], resumeDraft, (v) => { setResumeDraft(v); saveField({ resumeDraft: v }) }),
    ...
    function apply(ctx) {
      ...
      const call = (endpoint, payload) => connection.rpc.call(
        '/api', 'dsh-sticky-note/call', { endpoint, payload: payload || {} },
      ).catch(() => connection.rpc.call(CHANNEL, endpoint, payload || {}))
      const rpc = (endpoint, payload) => call(endpoint, payload).then((result) => {
        if (!result.ok) {
          const error = result.error || {}
          throw new Error(error.message || (typeof error.details === 'string' ? error.details : error.code) || 'rpc failed')
        }
        return result.value
      })
      ...
      const settingsCard = () => React.createElement(SettingsCard, {
        rpc,
        workspaces: ctx.get('uiWorkspace'),
      })
      slots.inject('plugins.bundle.config', () => slots.register(
        // 0.1.6-alpha.2 把外部包的设置页收口到插件管理器，key 必须是包名。
        { name: 'plugins.bundle.config', key: 'dsh-sticky-note' },
        ({ view }) => view === 'page' ? settingsCard() : null,
      ))
      // rc.7 ~ 0.1.1 仍由旧 keyed slot 托管设置卡片；新版没有该声明，inject 只会等待而不会注册。
    ...
    // 0.1.6 的浏览器 Loader 会按此声明创建可见服务的 Cordis fiber。
    // 没有这一项时模块会被下载，但 apply 无法拿到 slots/connection。
    exports.inject = ['connection', 'slots', 'uiWorkspace']
    exports.apply = apply
    exports.StickyNoteApp = StickyNoteApp
    exports.renderMarkdown = renderMarkdown
    exports.parsePanelLayout = parsePanelLayout
    exports.parsePanelZoom = parsePanelZoom
    exports.panelZoomCommand = panelZoomCommand
    exports.nextPanelZoom = nextPanelZoom
    exports.clampPanelLayout = clampPanelLayout
    exports.snapPanelLayout = snapPanelLayout
