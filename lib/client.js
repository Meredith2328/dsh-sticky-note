...
`
    ...
    function clampPanelLayout(layout, viewport) {
    ...
    function snapPanelLayout(layout, viewport) {
      const next = clampPanelLayout(layout, viewport)
      const left = PANEL_MARGIN
      const right = Math.max(PANEL_MARGIN, viewport.width - next.width - PANEL_MARGIN)
    ...
    function StickyNoteApp(props) {
      ...
      function togglePinned() {
        ...
        if (nextPinned) {
          const snapped = snapPanelLayout(layoutRef.current, viewport())
      ...
      function measurePanel() {
        ...
        const desired = Number.isFinite(preferred.x) && Number.isFinite(preferred.y)
          ? preferred
          : { ...preferred, x: r.right - preferred.width - 4, y: r.top - preferred.height - 8 }
        const next = pinnedRef.current
          ? snapPanelLayout(desired, viewport())
          : clampPanelLayout(desired, viewport())
      ...
      React.useEffect(() => {
        function onMove(e) {
          ...
          const next = clampPanelLayout(desired, viewport())
          preferredLayoutRef.current = next
          showLayout(next)
        }
        function onUp(e) {
          ...
          if (pinnedRef.current) {
            const snapped = snapPanelLayout(preferredLayoutRef.current, viewport())
            preferredLayoutRef.current = snapped
            showLayout(snapped)
          }
          persistLayout()
        ...
        function onResize() {
          const next = pinnedRef.current
            ? snapPanelLayout(preferredLayoutRef.current, viewport())
            : clampPanelLayout(preferredLayoutRef.current, viewport())
          preferredLayoutRef.current = next
          showLayout(next)
        }
      ...
      const panel = React.createElement('div', {
        ...
        style: Number.isFinite(layout.x) && Number.isFinite(layout.y)
          ? { left: layout.x + 'px', top: layout.y + 'px', width: layout.width + 'px', height: layout.height + 'px' }
          : { right: '16px', bottom: '68px', width: layout.width + 'px', height: layout.height + 'px' },
      },
