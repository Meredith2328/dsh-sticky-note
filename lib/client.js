...
.sn-openroot:hover:not(:disabled) {
  background: var(--dsw-alias-button-info-fill, #3b82f6);
  color: #fff;
}
.sn-openroot:active { transform: scale(0.94); }
.sn-openroot svg { display: block; }
...
`
    ...
    function StickyNoteApp(props) {
      ...
      const [openCats, setOpenCats] = React.useState({ '点子': true, '感想': false, 'TODO': false })
      ...
      // 自动保存定时器：按配置间隔定时保存当前内容（间隔 0 = 不自动保存）
      React.useEffect(() => {
        if (saveInterval <= 0) {
          if (saveTimerRef.current) clearInterval(saveTimerRef.current)
          return
        ...
        const headerRow = React.createElement('div', { className: 'sn-list-tools' },
          ...
          React.createElement('span', { className: 'sn-list-actions' },
            ...
            React.createElement('button', {
              ...
              onClick: () => {
                const next = {}
                for (const c of ['点子', '感想', 'TODO']) next[c] = !allOpen
                setOpenCats(next)
              },
            }, allOpen ? '全部收起' : '全部展开'),
          ),
              ...
              isOpen ? (items.length === 0
                ...
                : items.map((it) => {
                    ...
                    return React.createElement('div', {
                      ...
                      title: '单击 = 预备发送 · 双击 = 查看',
                      onClick: () => armSend({ kind: catName, name: it.name }),
      ...
      const panel = React.createElement('div', {
        ...
        React.createElement('div', { className: 'sn-head' },
          ...
          React.createElement('span', { className: 'sn-head-right' },
            ...
            React.createElement('button', { className: 'sn-help', title: '帮助', onClick: () => setHelpOpen(!helpOpen) }, '?'),
          ),
        ),
    ...
    function OpenRootButton(props) {
      const rpc = props.rpc
      const [title, setTitle] = React.useState('打开便签文件夹')
      return React.createElement('button', {
        type: 'button',
        className: 'sn-openroot',
        title: title,
        onClick: () => {
          rpc('openRoot', {}).then((v) => {
            if (v && v.root) { setTitle('已打开：' + v.root); setTimeout(() => setTitle('打开便签文件夹'), 2500) }
          }).catch(() => { setTitle('打开失败'); setTimeout(() => setTitle('打开便签文件夹'), 1500) })
        },
      }, React.createElement(FolderIcon, null))
    }
    ...
    function apply(ctx) {
      ...
      slots.inject('conversation.input.right', () => slots.register(
        { name: 'conversation.input.right', id: 'sticky-note-openroot', order: 90 },
        () => React.createElement(OpenRootButton, { rpc }),
      ))
